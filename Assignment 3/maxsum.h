...
struct sol
{
    ...
    void print() {cout << "Low: " << lo << " High: " << hi << " Sum: " << sum << endl;}
};

struct maxsums
{
    sol entire, begin, end, best;
    maxsums() {entire = sol(); begin = sol(); end = sol(); best = sol();}
    maxsums(sol s1, sol s2, sol s3, sol s4) {entire = s1; begin = s2; end = s3; best = s4;}
    void print();
};
